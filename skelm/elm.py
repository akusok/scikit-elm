...
class ELMClassifier(HiddenLayer, ClassifierMixin):
    ...
    def __init__(self, classes=None, solver="default", n_neurons=None, density=None, ufunc="tanh", pairwise_metric=None,
                 include_original_features=False, alpha=1e-7, random_state=None):
        HiddenLayer.__init__(self, n_neurons, density, ufunc, pairwise_metric, include_original_features, random_state)
        self.solver = solver
    ...
    def _prepare_model(self, X, y):
        ...
        if not hasattr(self, 'hidden_layer_'):
        ...
        if not hasattr(self, 'solver_'):
            if self.solver.lower() == 'ridge':
                ...
                raise ValueError("Unknow solver: {}".format(self.solver))
